...
module.exports = {
    requestOTP: (req,res) => {
        ...
        const htmlemail     = template({email,otp});
    ...
    //             },(error)=>{
    //                 if(error) return res.status(500).send({message:`requestOTP.b ${error.message}`});
    //                 return res.status(200).send({message:"OTP sent to your registered email. You have 5 minutes to verify your account, or you will have to request another OTP to verify your account"});
    //             });
    //         })
    //     })
    // },
